namespace LifelikeMotion.IKFootPlacement
{
    ...
    public class IKSetup : MonoBehaviour
    {
        ...
        public void FindReferences()
        {
            ...
            if (hips == null)
            {
                Transform hipsTransform = FindHierarchyTransform(hipsName);
                if (hipsTransform == null)
                {
                    Debug.LogError("Could not find hips transform \"" + hipsName + "\". Check the bone naming or assign the hips reference manually.");
                    return;
                }
            ...
            {
                iKLeg = new IKLeg();
                Transform leftUpLegTransform = FindHierarchyTransform(leftUpLegName);
                if (leftUpLegTransform != null)
                {
                    iKLeg.upLeg = leftUpLegTransform;
                    Transform leftLegTransform = FindHierarchyTransform(leftLegName);
                    if (leftLegTransform != null)
                    {
                        iKLeg.leg = leftLegTransform;
                        Transform leftFootTransform = FindHierarchyTransform(leftFootName);
                        if (leftFootTransform != null)
                        {
                            iKLeg.foot = leftFootTransform;
                ...
                {
                    iKLeg = new IKLeg();
                    leftUpLegTransform = FindHierarchyTransform(leftUpLegName + i);
                    if (leftUpLegTransform != null)
                    {
                        iKLeg.upLeg = leftUpLegTransform;
                    }
                    else break;
                    Transform leftLegTransform = FindHierarchyTransform(leftLegName + i);
                    if (leftLegTransform != null)
                    {
                        iKLeg.leg = leftLegTransform;
                    }
                    else break;
                    Transform leftFootTransform = FindHierarchyTransform(leftFootName + i);
                    if (leftFootTransform != null)
                    {
                        iKLeg.foot = leftFootTransform;
                    }
            ...
            {
                iKLeg = new IKLeg();
                Transform rightUpLegTransform = FindHierarchyTransform(rightUpLegName);
                if (rightUpLegTransform != null)
                {
                    iKLeg.upLeg = rightUpLegTransform;
                    Transform rightLegTransform = FindHierarchyTransform(rightLegName);
                    if (rightLegTransform != null)
                    {
                        iKLeg.leg = rightLegTransform;
                        Transform rightFootTransform = FindHierarchyTransform(rightFootName);
                        if (rightFootTransform != null)
                        {
                            iKLeg.foot = rightFootTransform;
                ...
                {
                    iKLeg = new IKLeg();
                    rightUpLegTransform = FindHierarchyTransform(rightUpLegName + i);
                    if (rightUpLegTransform != null)
                    {
                        iKLeg.upLeg = rightUpLegTransform;
                    }
                    else break;
                    Transform rightLegTransform = FindHierarchyTransform(rightLegName + i);
                    if (rightLegTransform != null)
                    {
                        iKLeg.leg = rightLegTransform;
                    }
                    else break;
                    Transform rightFootTransform = FindHierarchyTransform(rightFootName + i);
                    if (rightFootTransform != null)
                    {
                        iKLeg.foot = rightFootTransform;
                    }
        ...
        private Transform FindHierarchyTransform(string nameFragment)
        {
            ...
            Transform[] hierarchyTransforms = GetComponentsInChildren<Transform>(true);
            Transform containsMatch = null;

            for (int i = 0; i < hierarchyTransforms.Length; i++)
            {
                Transform currentTransform = hierarchyTransforms[i];
                if (string.Equals(currentTransform.name, nameFragment, System.StringComparison.OrdinalIgnoreCase))
                {
            ...
            {
                Transform currentTransform = hierarchyTransforms[i];
                if (currentTransform.name.IndexOf(nameFragment, System.StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    if (containsMatch == null || currentTransform.name.Length < containsMatch.name.Length)
                    {
                        containsMatch = currentTransform;
                    }
                }
            }
