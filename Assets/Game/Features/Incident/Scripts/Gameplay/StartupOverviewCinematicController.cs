...
public class StartupOverviewCinematicController : MonoBehaviour
{
    [Header("References")]
    [SerializeField] private StartupOverviewCameraPath cameraPath;
    ...
    private readonly System.Collections.Generic.List<Vector3> pathPositions = new System.Collections.Generic.List<Vector3>();
    private readonly System.Collections.Generic.List<Quaternion> pathRotations = new System.Collections.Generic.List<Quaternion>();
    private readonly System.Collections.Generic.List<CinemachineCamera> suppressedCinemachineCameras = new System.Collections.Generic.List<CinemachineCamera>();
    private readonly System.Collections.Generic.List<bool> suppressedCameraEnabledStates = new System.Collections.Generic.List<bool>();
    ...
    private IEnumerator PlayRoutine()
    {
        ...
        try
        {
            ...
            CreateOrRefreshRuntimeSkipButton();
            ...
            {
                runtimeCameraObject = new GameObject("StartupOverviewCinematicCamera");
            ...
            float skipEnableElapsed = 0f;
            float shotDuration = Mathf.Max(0.01f, totalPathDuration);
            float elapsed = 0f;
            while (elapsed < shotDuration)
            {
                elapsed += Time.unscaledDeltaTime;
                skipEnableElapsed += Time.unscaledDeltaTime;
                UpdateSkipAvailability(skipEnableElapsed);
                if (ConsumeSkipInput())
                {
                    break;
                }
    ...
    private void BuildPath(FirstPersonController playerController, Vector3 lookPosition)
    {
        ...
        StartupOverviewCameraPath resolvedPath = ResolveCameraPath();
    ...
    private void BuildWaypointPath(StartupOverviewCameraPath resolvedPath, Vector3 lookPosition)
    {
    ...
    private StartupOverviewCameraPath ResolveCameraPath()
    {
        if (cameraPath != null)
        {
            return cameraPath;
        ...
        cameraPath = FindAnyObjectByType<StartupOverviewCameraPath>(FindObjectsInactive.Exclude);
        return cameraPath;
    }
    ...
    private void RebuildSegmentDurations()
    {
        ...
        if (segmentCount <= 0)
        {
            segmentDurations = System.Array.Empty<float>();
    ...
    private void CreateOrRefreshRuntimeSkipButton()
    {
        if (!allowSkip)
        {
            ReleaseRuntimeSkipButton();
        ...
        runtimeSkipButtonText.raycastTarget = false;
    ...
    private void UpdateSkipAvailability(float elapsed)
    {
        ...
        {
            runtimeSkipCanvasObject.SetActive(allowSkip);
        }
    }
