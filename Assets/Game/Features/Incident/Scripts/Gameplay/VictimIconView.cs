...
public sealed class VictimIconView : MonoBehaviour
{
    ...
    private static Sprite GetFallbackSprite()
    {
        ...
        {
            name = "Runtime Victim Icon Texture",
        ...
        for (int y = 0; y < size; y++)
        {
            for (int x = 0; x < size; x++)
            {
                float u = (x + 0.5f) / size;
                float v = (y + 0.5f) / size;
                float head = 1f - Mathf.Clamp01((((u - 0.5f) / 0.18f) * ((u - 0.5f) / 0.18f)) + (((v - 0.68f) / 0.16f) * ((v - 0.68f) / 0.16f)));
                float torso = 1f - Mathf.Clamp01((Mathf.Abs(u - 0.5f) / 0.22f) + (Mathf.Abs(v - 0.34f) / 0.28f));
                float silhouette = Mathf.Clamp01(Mathf.Max(head, torso));
                float alpha = Mathf.SmoothStep(0f, 1f, silhouette);
                Color pixel = Color.Lerp(
                    new Color(0.9f, 0.14f, 0.14f, alpha),
                    new Color(1f, 0.9f, 0.9f, alpha),
                    Mathf.Clamp01(v));
        ...
        fallbackSprite.name = "Runtime Victim Icon Sprite";
        return fallbackSprite;
    }
}
