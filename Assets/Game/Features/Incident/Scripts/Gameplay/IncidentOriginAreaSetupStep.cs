...
public class IncidentOriginAreaSetupStep : IncidentMapSetupStep
{
    ...
    protected override IEnumerator Execute(IncidentMapSetupContext context)
    {
        ...
        {
            context.AddWarning(
                $"{nameof(IncidentOriginAreaSetupStep)}: Missing default fire prefab for payload application.",
                this);
            yield break;
        }
        ...
        {
            context.AddWarning(
                $"{nameof(IncidentOriginAreaSetupStep)}: Failed to resolve ignition source inside area '{area.name}'.",
                area);
            yield break;
        }

        if (!area.ApplyPayload(context.Payload, context.DefaultFirePrefab, source))
        {
            context.AddWarning(
                $"{nameof(IncidentOriginAreaSetupStep)}: Area '{area.name}' could not apply payload with resolved source '{source.DebugLabel}'.",
                area);
            yield break;
        }
        ...
        {
            context.AddWarning(
                $"{nameof(IncidentOriginAreaSetupStep)}: Payload requires isolation but area '{area.name}' has no linked " +
        ...
        {
            Debug.Log(
                $"{nameof(IncidentOriginAreaSetupStep)} resolved area '{area.name}' with source '{source.DebugLabel}' " +
